...
INSERT = 'INSERT INTO note (Jednostka, Kurs, Nazwa, Czas, timestamp) ' \
         'VALUES (:jednostka, :kurs, :nazwa, :czas, :timestamp)'




res = requests.get('http://notowania.pb.pl/commodities/')
...
def selector():
    selector_info = {
        "Jednostka": ["td", "colWaluta"],
        "Czas": ["td", "colAktualizacja"],
...
def data_in_db(pivoted_data):
    ...
    results = conn.execute("select jednostka, kurs, nazwa, czas, timestamp from note")
